/*
 * Hardcoded official baseball roster pages for every college on the board.
 * These are the school athletics sites where each player's profile lives
 * (roster → player page). Keyed by the exact `school` string used in
 * prospects.json. Kept here so PlayerProfile links directly instead of
 * routing through a web search.
 */
export const COLLEGE_SITES = {
  'Alabama':          'https://rolltide.com/sports/baseball/roster',
  'Arizona State':    'https://thesundevils.com/sports/baseball/roster',
  'Arkansas':         'https://arkansasrazorbacks.com/sport/m-basebl/roster/',
  'Auburn':           'https://auburntigers.com/sports/baseball/roster',
  'Cincinnati':       'https://gobearcats.com/sports/baseball/roster',
  'Coastal Carolina': 'https://goccusports.com/sports/baseball/roster',
  'East Carolina':    'https://ecupirates.com/sports/baseball/roster',
  'Florida':          'https://floridagators.com/sports/baseball/roster',
  'Florida State':    'https://seminoles.com/sports/baseball/roster',
  'Georgia':          'https://georgiadogs.com/sports/baseball/roster',
  'Georgia Tech':     'https://ramblinwreck.com/roster/?path=baseball',
  'Indiana State':    'https://gosycamores.com/sports/baseball/roster',
  'Kentucky':         'https://ukathletics.com/sports/baseball/roster',
  'LSU':              'https://lsusports.net/sports/bsb/roster',
  'Liberty':          'https://libertyflames.com/sports/baseball/roster',
  'Louisville':       'https://gocards.com/sports/baseball/roster',
  'McLennan JC (TX)': 'https://mclennanathletics.com/sports/bsb/index',
  'Mississippi':      'https://olemisssports.com/sports/baseball/roster',
  'Mississippi State':'https://hailstate.com/sports/baseball/roster',
  'Missouri State':   'https://missouristatebears.com/sports/baseball/roster',
  'NC State':         'https://gopack.com/sports/baseball/roster',
  'North Carolina':   'https://goheels.com/sports/baseball/roster',
  'Notre Dame':       'https://fightingirish.com/sports/baseball/roster/',
  'Oklahoma':         'https://soonersports.com/sports/baseball/roster',
  'Oregon State':     'https://osubeavers.com/sports/baseball/roster',
  'Rutgers':          'https://scarletknights.com/sports/baseball/roster',
  'Sam Houston State':'https://gobearkats.com/sports/baseball/roster',
  'TCU':              'https://gofrogs.com/sports/baseball/roster',
  'Tennessee':        'https://utsports.com/sports/baseball/roster',
  'Texas':            'https://texaslonghorns.com/sports/baseball/roster',
  'Texas A&M':        'https://12thman.com/sports/baseball/roster',
  'Texas Tech':       'https://texastech.com/sports/baseball/roster',
  'UC Santa Barbara': 'https://ucsbgauchos.com/sports/baseball/roster',
  'UCF':              'https://ucfknights.com/sports/baseball/roster',
  'UCLA':             'https://uclabruins.com/sports/baseball/roster',
  'USC':              'https://usctrojans.com/sports/baseball/roster',
  'Virginia':         'https://virginiasports.com/sports/baseball/roster',
  'Virginia Tech':    'https://hokiesports.com/sports/baseball/roster',
}
